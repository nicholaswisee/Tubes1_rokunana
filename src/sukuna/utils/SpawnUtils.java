...
public class SpawnUtils {
  public static UnitType decideTowerSpawn(RobotController rc,
      RobotInfo[] enemies,
      RobotInfo[] allies,
      MapInfo[] nearby,
      boolean opening,
      Random rng,
      int splasherPaintCost,
      int splasherChipCost,
      int splasherBankPaint,
      int splasherBankChips)
    ...
    boolean nearEnemySpace = enemyPaintNearCount >= 2 || enemyMopperNear > 0;
    boolean heavyEnemySpace = enemyPaintNearCount >= 4 || enemyMopperNear > 0;

    UnitType toSpawn = UnitType.SOLDIER;
    int roll = rng.nextInt(100);

    // Keep splasher logic as strategic expander (opening-biased + economy-aware).
    if (opening) {
  ...
  public static void buildWithPrimarySecondary(RobotController rc,
      UnitType first,
      UnitType second,
